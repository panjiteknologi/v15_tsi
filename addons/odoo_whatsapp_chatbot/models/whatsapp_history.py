...
class WhatsappHistory(models.Model):
    ...
    def create(self, vals):
        ...
        if vals.get("message") == None:
            pass
        else:
            received_message = vals.get("message").lower()
        ...
        if provider_id and partner_id and partner_id.mobile:
            ...
            if not channel:
                ...
                partner_id.write({
                        "channel_provider_line_ids": [(0,0,{"channel_id": channel.id,"provider_id": provider_id.id,},)]
                    })
            ...
            if (
                                    ...
                                    if component.interactive_product_list_ids:
                                        ...
                                        for (
                                            ...
                                            for products in product.product_list_ids:
                                                product_item = {"product_retailer_id": products.product_retailer_id}

                                                product_items.append(product_item)

                                            section.append(
                                                {"title": product.main_title,"product_items": product_items,}
                                            )
                                ...
                                if (provider_id.provider == "graph_api"):  # if condition for Graph API
                                    if (
                                        ...
                                        if self.env.context.get("wa_messsage_id"):
                                            self.env.context.get(
                                                "wa_messsage_id"
                                            ).wa_message_id = dict.get("messages")[0].get("id")
                                else:
                                    if "sent" in dict and dict.get("sent"):
                                        ...
                                        if not self.env.context.get("cron"):
                                            ...
                                            if "error" in dict:
                                                raise UserError(dict.get("error").get("message"))
                                        else:
                ...
                if provider_id and partner_id and partner_id.mobile:
                    ...
                    if vals.get("type") == "received":
                        ...
                        message_script = (
                            ...
                            .filtered(lambda l: l.message.lower() == received_message)
                        )
                        
                        # _logger.info("Pesan Customer (Setelah Diproses): %s", received_message)
                        # _logger.info("Pesan Chatbot yang Ditemukan: %s", [msg.message for msg in message_script])
                        
                        # _logger.info("Pesan Chatbot yang Ditemukan Dengan Lower: %s", [msg.message.lower() for msg in message_script if msg.message])
                            ...
                            if "quotedMsgId" in self.env.context:
                                ...
                                if len(parent_message) > 0:
                                    ...
                                    if (
                                        ...
                                        notifications = (
                                            channel._channel_message_notifications(chatter_wa_message)
                                        )
                        ...
                        if not self.env.context.get("whatsapp_application"):
                            if ("template_send" in self.env.context and self.env.context.get("template_send")):
                                            ...
                                            for variable in component.variables_ids:
                                                ...
                                                if variable.field_id.ttype == "text":
                                                    if object_data.get(variable.field_id.name):
                                                        ...
                                                        parameter_dict.update({"type": "text", "text": ""})
                                                ...
                                                if (variable.field_id.ttype == "monetary"):
                                                    ...
                                                    if "currency_id" in object_data:
                                                        ...
                                                        if currency.position == "after":
                                                            text = (str(object_data.get(variable.field_id.name)) + currency.symbol)
                                                        else:
                                                            text = currency.symbol + str(object_data.get(variable.field_id.name))
                                                    parameter_dict.update(
                                                        {"type": "text", "text": text}
                                                    )
                                                ...
                                                if variable.field_id.ttype in ["date"]:
                                                    if object_data.get(variable.field_id.name):
                                                        parameter_dict.update(
                                                            {
                                                                "type": "text",
                                                                "text": object_data.get(variable.field_id.name).strftime("%m/%d/%Y"),
                                                            }
                                                        )
                                                    else:
                                                        parameter_dict.update(
                                                ...
                                                if variable.field_id.ttype == "html":
                                                    if object_data.get(variable.field_id.name):
                                                        parameter_dict.update(
                                                            {
                                                                "type": "text",
                                                                "text": tools.html2plaintext(object_data.get(variable.field_id.name)),
                                                            }
                                                        )
                                                    else:
                                                        parameter_dict.update(
                                            ...
                                            if (component.formate_media_type == "dynamic"):
                                                        ...
                                                        parameters = [
                                                            {
                                                                ...
                                                                "document": {
                                                                    "link": base_url
                                                                    + "/web/content/"
                                                                    + str(self.env.context.get("attachment_ids")[0]),
                                                        ...
                                                        parameters = [
                                                            {
                                                                ...
                                                                "document": {
                                                                    "link": base_url
                                                                    + "/web/content/"
                                                                    + str(component.attachment_ids.ids[0]),
                                                                    "filename": self.env["ir.attachment"]
                                                                    .sudo()
                                                                    .browse(component.attachment_ids.ids[0])
                                                                    .name,
                                                                },
                                                            }
                                                        ]
                                                ...
                                                if component.media_type == "image":
                                                    if component.attachment_ids:
                                                        ...
                                                        _logger.info("Image Parameters: %s", parameters)
                                                        template_dict.update({"parameters": parameters})

                                    if component.type == "buttons":
                                        if (
                                            component.button_type == "call_to_action"
                                            and (component.type_of_action or component.type_of_action_2) == "URL"
                                            and (component.url_type or component.url_type_2) == "dynamic"):
                                            ...
                                            if component.variables_ids:
                                                ...
                                                for variable in component.variables_ids:
                                                    ...
                                                    if (variable.field_id.ttype == "monetary"):
                                                        ...
                                                        if currency.position == "after":
                                                            text = (
                                                                str(object_data.get(variable.field_id.name))
                                                                + currency.symbol
                                                            )
                                                        else:
                                                            text = currency.symbol + str(object_data.get(variable.field_id.name))
                                            ...
                                            template_dict.update(
                                                {
                                                    ...
                                                    "parameters": [
                                                        {
                                                            "type": "action",
                                                            "action": {"flow_token": component.flow_id,},
                                                        }
                                                    ],
                                                }
                                            )
                                            ...
                                            template_dict.update(
                                                {
                                                    ...
                                                    "parameters": [
                                                        {
                                                            "type": "action",
                                                            "action": {"thumbnail_product_retailer_id": component.product_retailer_id},
                                                        }
                                                    ],
                                                }
                                            )
                                ...
                                if answer.status_code == 200:
                                    dict = json.loads(answer.text)
                                    _logger.info("Data Answer file Whatsapp_history %s",dict)
                                    if (provider_id.provider == "graph_api"):  # if condition for Graph API
                                        if (
                                            "messages" in dict
                                            and dict.get("messages")
                                ...
                                if received_message:    
                                    ...
                                    if "message_parent_id" in self.env.context:
                                        parent_msg = (
                                            self.env["mail.message"]
                                            .sudo()
                                            .search([("id","=",self.env.context.get("message_parent_id").id,)])
                                        )
                                ...
                                if vals.get("attachment_ids"):
                                    for attachment in vals.get("attachment_ids"):
                                        if attachment[1]:
                                            ...
                                            _logger.info("Data Gambar: %s", attachment_id)
                                            ...
                                            if provider_id.provider == "graph_api":
                                                ...
                                                if answer.status_code == 200:
                                                    ...
                                                    getimagebyid = (
                                                        provider_id.get_image_by_id(media_id,partner_id,sent_type,attachment_id,)
                                                    )
                                                    ...
                                                    if ("messages" in imagedict and imagedict.get("messages")):
                                                        ...
                                                        if not self.env.context.get("cron"):
                                                            if "messages" in imagedict:
                                                                raise UserError(imagedict.get("message"))
                                                            if "error" in imagedict:
                                                                raise UserError(imagedict.get("error").get("message"))
                                                        else:
                                                            vals.update({"type": "fail"})
                                                            if "messages" in imagedict:
                                                                vals.update({
                ...
                if received_message and vals.get("type") == "received":
                # if vals.get("message") and vals.get("type") == "received":

                    if provider_id.company_id and provider_id.company_id.wa_chatbot_id:
                        # Bot

                        if (provider_id.company_id.wa_chatbot_id and not channel.is_chatbot_ended):
                            message_script = (
                                self.env["whatsapp.chatbot"]
                            ...
                            for chat in chatbot_script_lines:
                                ...
                                if chat.step_call_type == "message":
                                    ...
                                    if not self.env.context.get("whatsapp_application"):
                                        ...
                                        notifications = (
                                            channel._channel_message_notifications(message)
                                        )
                                        self.env["bus.bus"]._sendmany(notifications)
                                        ...
                                        if answer.status_code == 200:
                                            dict = json.loads(answer.text)
                                            if (
                                                provider_id.provider == "graph_api"):  # if condition for Graph API
                                                if (
                                                    "messages" in dict
                                                    and dict.get("messages")
                                                    and dict.get("messages")[0].get("id")
                                ...
                                if chat.step_call_type == "interactive":
                                    ...
                                    if not self.env.context.get("whatsapp_application"):
                                        ...
                                        notifications = (
                                            channel._channel_message_notifications(wa_message_body)
                                        )
                                        self.env["bus.bus"]._sendmany(notifications)
                                ...
                                if chat.step_call_type == "action":
                                    ...
                                    if chat.action_id:
                                        ...
                                        #     if active_operator:
                                        #         wa_chatbot_channels = provider_id.company_id.wa_chatbot_id.mapped("channel_ids")
                                        ...
                                        #                         dict["id"]
                                        #                         if self.env.context.get("wa_messsage_id"):
                                        #                             self.env.context.get("wa_messsage_id"
                                        #                             ).wa_message_id = dict["id"]
                                        #                     else:
                                        #                         if not self.env.context.get("cron"):
                                        #                             if ("message" in dict):
                                        ...
                                        if (chat.action_id.binding_model_id.model == "mail.channel"):
                                            ...
                                            active_operator = provider_id.company_id.wa_chatbot_id.mapped("user_ids")
                                            # _logger.info("active_operator:%s", active_operator)
                                            
                                            if active_operator:
                                                wa_chatbot_channels = provider_id.company_id.wa_chatbot_id.mapped("channel_ids")
                                                
                                                for active in active_operator:
                                                    for wa_channel in wa_chatbot_channels:
                                                        operators = active.filtered(
                                                            lambda av_user: av_user.partner_id
                                                            not in wa_channel.channel_last_seen_partner_ids.partner_id
                                                        )
                                                        if operators:
                                                            available_operators.update(operators.mapped("partner_id"))
                                                        else:
                                                            available_operators.add(active.partner_id)

                                                if available_operators:
                                                    if channel.whatsapp_channel:
                                                        channel.write(
                                                            {
                                                                "channel_partner_ids": [(4, operator.id) for operator in available_operators],
                                                                "is_chatbot_ended": True,
                                                                "wa_chatbot_id": False,
                                                            }
                                                        )
                                                    ...
                                                    user_message = "This chat is redirected to our customer service"
                                                    ...
                                                    message_values = {
                                                        ...
                                                        "partner_ids": [(4, operator.id) for operator in available_operators],
                                                        "res_id": channel.id,
                                                        "reply_to": partner_id.email,
                                                        "company_id": vals.get("company_id"),
                                                        "wa_chatbot_id": chat.whatsapp_chatbot_id.id,
                                                    ...
                                                    message = (
                                                        self.env["mail.message"].sudo().with_context({"provider_id": provider_id}).create(message_values)
                                                    )
                                                    notifications = channel._channel_message_notifications(message)
                                                    self.env["bus.bus"]._sendmany(notifications)
                                                    ...
                                                    if answer.status_code == 200:
                                                        ...
                                                        if provider_id.provider == "graph_api":
                                                            if "messages" in dict_response and dict_response.get("messages"):
                                                                dict_response.get("messages")[0].get("id")
                                                        else:
                                                            if dict_response.get("sent"):
                                                                if self.env.context.get("wa_messsage_id"):
                                                                    self.env.context["wa_messsage_id"].wa_message_id = dict_response["id"]
                                                            else:
                                                                if not self.env.context.get("cron"):
                                                                    if "message" in dict_response:
                                                                        raise UserError(dict_response.get("message"))
                                                                    if "error" in dict_response:
                                                                        raise UserError(dict_response.get("error").get("message"))
                                                                else:
                                                                    vals.update({"type": "fail"})
                                                                    if "error" in dict_response:
                                                                        vals.update({"fail_reason": dict_response.get("error").get("message")})

                                            else:
                                                user_message = "Sorry, no active Operator currently available"
                                                message_values = {
                                                    "body": "<p> " + user_message + "</p>",
                                                    "author_id": user_partner.id,
                                                    "email_from": user_partner.email or "",
                                                    "model": "mail.channel",
                                                    "message_type": "wa_msgs",
                                        ...
                                        if (chat.action_id.binding_model_id.model == "tsi.iso"):
                                            nomor=partner_id.mobile
                                            _logger.info("NOMOR :%s",nomor)
                                            payload = {
                                                    "messaging_product": "whatsapp",
                                                    "to": nomor,
                                                    "type": "template",
                                                    "template": {
                                                        # "name": "form_iso_tes", #dev
                                                        "name": "form_iso_prod", #prod
                                                        "language": {
                                                            "code": "en_US"
                                                        },
                                                        "components": [
                                        ...
                                        if (chat.action_id.binding_model_id.model == "tsi.audit_request_chatbot"):
                                            nomor=partner_id.mobile
                                            _logger.info("NOMOR :%s",nomor)
                                            payload = {
                                                    "messaging_product": "whatsapp",
                                                    ...
                                                    "type": "template",
                                                    "template": {
                                                        # "name": "audit_request_tes", #dev
                                                        "name": "audit_request_prod", #prod
                                                        "language": {
                                                            "code": "en_US"
                                                        },
                                                        "components": [
                                        ...
                                        if (chat.action_id.binding_model_id.model == "tsi.partner_feedback"):
                                            nomor=partner_id.mobile
                                            _logger.info("NOMOR :%s",nomor)
                                            payload = {
                                                    "messaging_product": "whatsapp",
                                                    ...
                                                    "type": "template",
                                                    "template": {
                                                        # "name": "ticket_complain_tes", #dev
                                                        "name": "ticket_complain_prod", #prod
                                                        "language": {
                                                            "code": "en_US"
                                                        },
                                                        "components": [
