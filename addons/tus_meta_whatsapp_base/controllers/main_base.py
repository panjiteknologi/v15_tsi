...
class WebHook(http.Controller):
    ...
    def whatsapp_webhook(self, **kw):
        ...
        provider = (
            ...
            .sudo()
            .search([("chat_api_instance_id", "=", data.get("instanceId"))], limit=1)
        )
        ...
        if "ack" in data and data.get("ack"):
            for acknowledgment in data.get("ack"):
                wp_msgs = (
                    ...
                    .sudo()
                    .search([("message_id", "=", acknowledgment.get("id"))], limit=1)
                )
                if wp_msgs:
                                ...
                                if mes["type"] == "chat":
                                    ...
                                    if mes["quotedMsgId"] != None:
                                        request.env[
                                            ...
                                            {"quotedMsgId": mes.get("quotedMsgId")}
                                        ).create(
                                            vals
                                        )
                                    ...
                                    datas = base64.b64encode(
                                        requests.get(mes.get("body")).content
                                    )
                                ...
                                if channel:
                                    ...
                                    if mes["type"] == "location":
                                        ...
                                        message_values.update(
                                            {
                                                "body": "<a href='https://www.google.com/maps/search/?api=1&query="
                                                + str(latlng[0])
                                                + ","
                                                + str(latlng[1])
                                                + "' target='_blank' class='btn btn-primary'>Google Map</a>"
                                            ...
                                            if (
                                                ...
                                                if message_values.get("attachment_ids"):
                                                    chatter_wa_message_values.update(
                                                        {
                                                            "body": "",
                                                            "attachment_ids": message_values.get(
                                                                "attachment_ids"
                                                            ),
                                                        }
                                                    )
