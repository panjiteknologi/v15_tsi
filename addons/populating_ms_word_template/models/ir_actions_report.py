...
#     def export_doc_by_template(self, file_template_data=None, suffix='docx', file_name_export='export1', datas={}):
#         if not file_template_data and not self.file_template_data:
#             raise ValueError("No DOCX template provided for the report.")
        
#         file_template_data = file_template_data or self.file_template_data
#         file_template = self._convert_binary_to_doc(file_template_data=file_template_data, suffix=suffix)
#         document = MailMerge(file_template.name)
#         fields = document.get_merge_fields()
...
#                     value = self._convert_datetime_usertz_to_utctz(data)
#                 elif isinstance(data, date):
#                     value = data.strftime(self.env['res.lang'].search([('code', '=', self.env.user.lang)], limit=1).date_format)
#                 elif isinstance(data, bool):
#                     value = '' if not data else str(data)
...
#         if self.file_template_name:
#             if not populating_tables:
#                 document.merge(**simple_merge)
#                 result = BytesIO()
...
#             else:
#                 for key, values in populating_tables.items():
#                     document.merge_pages(values)
#                 result = BytesIO()
#                 document.write(result)
#                 result.seek(0)
#                 return result.getvalue()
...
#     def _convert_datetime_usertz_to_utctz(self, dt):
#         if not dt.tzinfo:
#             dt = pytz.UTC.localize(dt)
#         return dt.astimezone(pytz.UTC).strftime(self.env['res.lang'].search([('code', '=', self.env.user.lang)], limit=1).datetime_format)
