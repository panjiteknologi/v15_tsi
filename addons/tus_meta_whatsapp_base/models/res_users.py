...
class ResUsers(models.Model):
    ...
    provider_id = fields.Many2one(
        "provider", "Provider", domain="[('company_id', 'in', company_ids)]"
    )
    ...
    def write(self, values):
        ...
        if values.get("groups_id"):
            ...
            uncheck_ids = []
            whatsapp_manager_group = self.env.ref(
                "tus_meta_whatsapp_base.whatsapp_group_manager"
            ).id
            inherited_groups = self.env["res.groups"].search(
                [
                    (
                        "implied_ids",
                        "in",
                        self.env.ref("tus_meta_whatsapp_base.whatsapp_group_user").ids,
                    )
                ]
            )
            for group in groups:
