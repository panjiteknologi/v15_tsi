...
class WATemplate(models.Model):
    ...
    def init(self):
        video_path = get_module_resource(
            "tus_meta_whatsapp_base", "static/src/video", "wa-demo-video.mp4"
        )
        attachment = (
            self.env["ir.attachment"].sudo().search([("name", "=", "demo-wa-video")])
        )
        if not attachment:
            attachment_value = {
                "name": "demo-wa-video",
        ...
        pdf_path = get_module_resource(
            "tus_meta_whatsapp_base", "static/src/pdf", "TestPDFfile.pdf"
        )
        attachment = (
            self.env["ir.attachment"].sudo().search([("name", "=", "demo-wa-pdf")])
        )
        if not attachment:
            attachment_value = {
                "name": "demo-wa-pdf",
        ...
        pdf_path = get_module_resource(
            "tus_meta_whatsapp_base", "static/src/image", "whatsapp_default_set.png"
        )
        attachment = (
            self.env["ir.attachment"].sudo().search([("name", "=", "demo-wa-image")])
        )
        if not attachment:
            attachment_value = {
                "name": "demo-wa-image",
    ...
    provider_id = fields.Many2one(
        "provider", "Provider", default=_get_current_user_provider
    )  # default=lambda self: self.env.user.provider_id
    ...
    show_graph_message_template_id = fields.Boolean(
        compute="_compute_show_graph_message_template_id"
    )
    ...
    def onchange_name(self):
        for rec in self:
            if rec.name and not re.match("^[A-Za-z0-9_]*$", rec.name):
                raise UserError(
                    _("Template name contains letters, numbers and underscores.")
                )
    ...
    def add_whatsapp_template(self):
        ...
        for component in self.components_ids:
            ...
            if component.type == "header":
                if component.formate == "media":
                    ...
                    IrConfigParam.get_param("web.base.url", False)
                    attachment = False
                    ...
                    if component.media_type == "image":
                        attachment = (
                            self.env["ir.attachment"]
                            .sudo()
                            .search([("name", "=", "demo-wa-image")])
                        )

                    if attachment:
                        answer = self.provider_id.graph_api_upload_demo_document(
                            attachment
                        )
                            ...
                            dict.update(
                                {
                                    ...
                                    "format": component.media_type.upper(),
                                }
                            )
                    ...
                    if header_text:
                        dict.update({"example": {"header_text": header_text}})
                    if component.text:
                        dict.update(
                            {
                                "text": component.text,
                ...
                if component.button_type == "call_to_action":
                    if (
                        component.type_of_action == "PHONE_NUMBER"
                        or component.type_of_action == "URL"
                    ) and component.is_button_clicked:
                        ...
                        component.type_of_action == "PHONE_NUMBER"
                        or component.type_of_action == "URL"
                    ) and not component.is_button_clicked:
                        if (
                            component.type_of_action == "PHONE_NUMBER"
                            and component.type_of_action_2 == "PHONE_NUMBER"
                        ):
                            dict.update(
                                {
                                    "type": component.type.upper(),
                                    "buttons": [
                                        ...
                                        {
                                            "type": component.type_of_action_2,
                            ...
                            component.type_of_action == "PHONE_NUMBER"
                            and component.type_of_action_2 == "URL"
                        ):
                            ...
                            component.type_of_action == "URL"
                            and component.type_of_action_2 == "URL"
                        ):
                            if (
                                component.url_type == "static"
                                and component.url_type_2 == "static"
                            ):
                                dict.update(
                                    {
                                        "type": component.type.upper(),
                                ...
                                component.url_type == "static"
                                and component.url_type_2 == "dynamic"
                            ):
                                dict.update(
                                    {
                                        "type": component.type.upper(),
                    ...
                    if (
                        component.quick_reply_type == "custom"
                        and component.is_button_clicked == True
                        and component.is_second_button_clicked == True
                    ):
                        ...
                        component.quick_reply_type == "custom"
                        and component.is_button_clicked == False
                        and component.is_second_button_clicked == True
                    ):
                        ...
                        component.quick_reply_type == "custom"
                        and component.is_button_clicked == True
                        and component.is_second_button_clicked == False
                    ):
                ...
                if body_text:
                    dict.update({"example": {"body_text": [body_text]}})
                if component.text:
                    dict.update(
                        {
                            "text": component.text,
        ...
        if components:
            ...
            answer = self.provider_id.add_template(
                self.name, self.lang.iso_code, self.category.upper(), components
            )
            if answer.status_code == 200:
    ...
    def resubmit_whatsapp_template(self):
        ...
        if components:
            answer = self.provider_id.resubmit_template(
                self.category.upper(), self.graph_message_template_id, components
            )
            if answer.status_code == 200:
                dict = json.loads(answer.text)
                            ...
                            if template.get("status") in ["approved", "submitted"]:
                                ...
                                for comp in template.get("components"):
                                    ...
                                    if component_type == "HEADER":
                                        if (
                                            comp_media_type == "VIDEO"
                                            or "DOCUMENT"
                                            or "IMAGE"
                                            or "TEXT"
                                        ):
                                            if comp_media_type == "TEXT":
                                                compoment_ids = self.env[
                                                    "components"
                                                ].create(
                                                    {
                                                        "type": comp.get(
    ...
    def get_whatsapp_template_status(self):
        if self.provider_id.graph_api_authenticated:
            base_url = (
                self.provider_id.graph_api_url
                + self.provider_id.graph_api_business_id
                + "/message_templates?name="
                + self.name
                + "&access_token="
                + self.provider_id.graph_api_token
            )
            headers = {"Authorization": self.provider_id.graph_api_token}
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if response_data["data"] and response_data["data"][0]:
                        ...
                        print(response_data)
                        res = self.write({"template_status": status})
    ...
    def get_all_templates_status(self):
        ...
        for rec in templates:
            if rec.provider_id.graph_api_authenticated:
                base_url = (
                    rec.provider_id.graph_api_url
                    + rec.provider_id.graph_api_business_id
                    + "/message_templates?name="
                    + rec.name
                    + "&access_token="
                    + rec.provider_id.graph_api_token
                )
                ...
                if response.status_code == 200:
                    ...
                    if response_data["data"] and response_data["data"][0]:
                        status = response_data["data"][0]["status"]
                        rec.write({"template_status": status})
