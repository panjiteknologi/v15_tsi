...
class WAComposer(models.TransientModel):
    ...
    domain_template_ids = fields.Many2many(
        comodel_name="wa.template", string="domain template ids"
    )
    attachment_ids = fields.Many2many(
        "ir.attachment",
        "wa_compose_message_ir_attachments_rel",
        "wa_wizard_id",
        "attachment_id",
        "Attachments",
    )
        ...
        if "active_model" in self.env.context:
            ...
            for record in self:
                if record.template_id:
                    if record.template_id.components_ids.filtered(
                        lambda comp: comp.type == "body"
                    ):
                        variables_ids = record.template_id.components_ids.variables_ids
                        if variables_ids:
                            temp_body = tools.html2plaintext(
                                record.template_id.body_html
                            )
                            variables_length = len(
                                record.template_id.components_ids.variables_ids
                            )
                            for length, variable in zip(
                                range(variables_length), variables_ids
                            ):
                                st = "{{%d}}" % (length + 1)
                            ...
                            record.body = (
                                record.body
                            ) = record.template_id._render_field(
            ...
            self.partner_id.write(
                {
                    "channel_provider_line_ids": [
                        (
                            0,
                            0,
                            {"channel_id": channel.id, "provider_id": provider_id.id},
                        )
                    ]
                }
            )
        ...
        if channel:
            if self.template_id:
                ...
                message = (
                    self.env["mail.message"]
                    .sudo()
                    .with_context({"provider_id": self.provider_id})
