...
class Message(models.Model):
    ...
    chatter_wa_res_id = fields.Many2oneReference(
        "Chatter Wa Res Id", index=True, model_field="chatter_wa_model"
    )
    ...
    def create(self, values_list):
        ...
        for values in values_list:
            ...
            if (
                values.get("model") == "mail.channel"
                and channel_id.whatsapp_channel
                and is_tus_discuss_installed
            ):
                    ...
                    if user.id != self.env.ref("base.public_user").id:
                        if values.get("model") == "mail.channel":
                            ...
                            if values.get("model") == "res.partner":
                                ...
                                if data[0]["partner_id"]:
                                    ...
                                    raise AccessError(
                                        _(
                                            "Partner must be Required for whatsapp message!"
                                        )
                                    )
                            else:
                                raise AccessError(
                        ...
                        if self.env.context.get("template_send"):
                            ...
                            if self._context.get("active_model_id"):
                                vals.update(
                                    {"rec_id": self._context.get("active_model_id")}
                                )
                            self.env["whatsapp.history"].sudo().with_context().create(
                                vals
                            )
        return messages
